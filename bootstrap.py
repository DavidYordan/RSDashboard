...
def create_directory(path):
    final_path = os.path.join(path, 'RSDashboard')
    if not os.path.exists(final_path):
        os.makedirs(final_path)
    return final_path

def create_shortcut_with_vbscript(working_dir, target, shortcut_name, shortcut_dir):
    vbscript = f"""
...
def extract_resources(source_dir, target_dir):
    ...
    for resource_path in resource_paths:
        ...
        if resource_path == 'dist':
            shutil.copy(
                os.path.join(full_source_path, 'RSDashboard.exe'),
                os.path.join(target_dir, 'RSDashboard.exe')
            )
        else:
...
def main():
    ...
    exe_path = os.path.join(working_dir, 'RSDashboard.exe')
    
    create_shortcut_with_vbscript(working_dir, exe_path, 'RSDashboard', get_desktop_path())
